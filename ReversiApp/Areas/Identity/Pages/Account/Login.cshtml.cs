    ...
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            try
            {
                ...
                {
                    this.ModelState.AddModelError(string.Empty, "Problem with the ReCaptcha. Please try again.");
                }
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    result = await ((UsernameEmailSigninManager)_signInManager).PasswordSignBothInAsync(Input.Email, Input.Password, Input.RememberMe, lockoutOnFailure: false);
                    if (result.Succeeded)
                    {
                        _logger.LogInformation("User logged in.");
                        return LocalRedirect(returnUrl);
